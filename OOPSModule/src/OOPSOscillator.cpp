...
static void tSawtoothReset(tSawtooth* t)
{
    t->inc      = 0.0f;
    t->phase    = 0.0f;
    
    t->sampleRateChanged = &tSawtoothSampleRateChanged;
}
...
tSawtooth*    tSawtoothInit(void)
{
    ...
    tSawtooth* t = &oops.tSawtoothRegistry[oops.registryIndex[T_SAWTOOTH]++];
...
oSawtooth::oSawtooth (void)
{
    tSawtoothReset(this);
}


float oSawtooth::tick (void)
{
    float sample = tSawtoothTick(this);

    return sample;
}
    
void oSawtooth::setFreq(float freq)
{
...
int     tSawtoothSetFreq(tSawtooth* const c, float freq)
{
    ...
    c->freq = freq;
    c->inc = freq * oops.invSampleRate; //44100.0f;
